...
router.get('/:id_eva',verifyToken,async (req,res) => {
    try{
        ...
        const [rows] = await db.query(`select * from tb_member m,tb_system s,tb_eva e,tb_commit c where c.id_member=? and c.id_eva=? and c.id_eva=e.id_eva and e.id_member=m.id_member and e.id_sys=s.id_sys and status_sys=?`,
            [id_member,id_eva,'y']
        )
        res.json(rows[0])
    }catch(err){
...
router.get('/indicate/:id_eva',verifyToken,async (req,res) => {
    try{
        ...
        const [indicates] = await db.query(`select * from tb_indicate i,tb_evadetail d where i.id_indicate=d.id_indicate and d.id_eva=? and d.status_eva=?`,
            [id_eva,1]
        )
...
router.post('/:id_eva',verifyToken,async (req,res) => {
    try{
        ...
        const [[commitRow]] = await db.query(`select * from tb_member m,tb_system s,tb_eva e,tb_commit c where c.id_member=? and c.id_eva=? and c.id_eva=e.id_eva and e.id_member=m.id_member and e.id_sys=s.id_sys and status_sys=?`,
            [id_member,id_eva,'y']
        )
        var statusCommit = 0
        ...
        const [[sumRow]] = await db.query(
            ...
            from tb_evadetail d where id_eva=?`,
            [id_eva]
        )
        await db.query(`update tb_eva set total_commit=? where id_eva=?`,
            [sumRow.total,id_eva]
        )
        await db.query(`update tb_commit set status_commit=?,detail_commit=? where id_eva=? and id_member=?`,
            ['y',detail_commit,id_eva,id_member]
        )
        const [rowCommit] = await db.query(`select * from tb_commit c where c.id_eva=? and status_commit=?`,
            [id_eva,'y']
        )
        if(rowCommit.length === 3){
